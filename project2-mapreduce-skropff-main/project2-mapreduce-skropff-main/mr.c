...
void *mapper_prepare(void *arg) {
  ...
  while (current != NULL) {
    // start->value = (char *) realloc(start->value, strlen(statt->value) +
    // strlen(current->value) + 2); strcat(start->value, space);
...
for (int i = 0; i < (int) num_mapper; i = i + 1) {
  ...
  while (current != NULL) {
    if (current != start) {
      start->value = (char *) realloc(start->value, strlen(statt->value) +
strlen(current->value) + 2); strcat(start->value, space); strcat(start->value,
current->value);
    }
  ...
  */
  ...
  for (int i = 0; i < (int)num_mapper; i = i + 1) {
    ...
    pthread_create(mapper_id + i, NULL, (void *(*)(void *)) & mapper_prepare,
                   (void *)(input1 + i));
  }
  for (int i = 0; i < (int)num_mapper; i = i + 1) {
  ...
  for (int i = 0; i < (int)num_mapper; i = i + 1) {
    ...
    while (current != NULL) {
      kvlist_append(lists3[(hash(get_kv(current)->key)) % num_reducer],
                    get_kv(current));
      // printf("appending 1\n");
      current = get_next(current);
    }
  ...
  for (int i = 0; i < (int)num_reducer; i = i + 1) {
    ...
    pthread_create(reducer_id + i, NULL, (void *(*)(void *)) & reducer_prepare,
                   (void *)(input2 + i));
  }
  // printf("Bench4\n");
  for (int i = 0; i < (int)num_reducer; i = i + 1) {
